...
**Current Release:** Version 2026.05.05.15 (Published: 05.05.2026 15:52)
...
- **Dialogs Page**: Installer-dialogs configuration page (toolbar index 4) — one row per dialog type (Welcome, License, Dependencies, For Me/All Users, Components, Shortcuts, Ready to Install, Install, Finish) with 32×32 shell32.dll icon, type name or enable checkbox, "Edit Content…" and "Preview…" buttons per row. The four always-present rows — Welcome, License, Ready to Install, and Finish — each have a **per-dialog enable checkbox** (`IDC_IDLG_ROW_ENABLE_BASE + type`, range 7060–7068) that replaces the plain name label; when unchecked the dialog is excluded from the installer script and skipped in preview navigation. `IDLG_IsDialogEnabled(InstallerDialogType)` public accessor returns the toggle state for script generation (emit `DisableWelcomePage=yes`, `DisableReadyPage=yes`, `DisableFinishedPage=yes` in Inno `[Setup]`). The Install row (progress bar) cannot be disabled. Conditional rows (Dependencies, For Me/All Users, Components, Shortcuts) are governed by their own project-state logic and keep a plain label. **Ready row — "Show summary of choices" sub-section**: `IDC_IDLG_READY_SHOW_DIR` (7074) and `IDC_IDLG_READY_SHOW_GROUP` (7075) checkboxes control `AlwaysShowDirOnReadyPage` and `AlwaysShowGroupOnReadyPage` in the Inno `[Setup]` section; both default to checked (matching Inno's own defaults). Public accessors `IDLG_GetReadyShowDir()` / `IDLG_GetReadyShowGroup()` for script generation. **Finish row — "Launch app when done" sub-section**: a `IDC_IDLG_FINISH_LAUNCH` checkbox (default off) causes SetupCraft to emit a `[Run]` entry (`Filename: "{app}\{#MyAppExeName}"; Flags: nowait postinstall shellexec skipifsilent`) so the installer offers a "Launch AppName now" checkbox on its own Finish page; the description text is editable (`IDC_IDLG_FINISH_LAUNCH_DESC`, default `Launch {#MyAppName}`) and a "Default checked" toggle (`IDC_IDLG_FINISH_LAUNCH_DEFCHK`, default on) controls whether Inno adds `Flags: unchecked`. Public accessors: `IDLG_GetFinishLaunchEnabled()`, `IDLG_GetFinishLaunchDesc()`, `IDLG_GetFinishLaunchDefaultChecked()`. License row has three sub-controls: a **"License source:"** drop-down combo (`IDC_IDLG_LICENSE_SRC = 7050`) that switches between **Built-in RTF editor** (default — full RTF editor with template pre-fill) and **External file** (browse to an existing `.rtf` or `.txt` file; path stored in `s_licenseFilePath`, persisted as `installer_license_file_<pid>`; "Edit Content…" is disabled in this mode); a **"License template:"** combo (`IDC_IDLG_LICENSE_TEMPLATE = 7047`) visible only in Built-in mode — pre-fills content from 19 built-in templates with `<<AppName>>` and `<<Year>>` substituted; and a custom checkbox `IDC_IDLG_LICENSE_ACCEPT = 7046` (default checked) — when enabled the License preview shows a radio-button pair (*I accept* / *I do not accept*, default = do not accept) so the end user must actively accept before Next is enabled; `IDLG_GetLicenseMustAccept()`, `IDLG_GetLicenseSource()`, `IDLG_GetLicenseFilePath()` expose state to script generation. Installer-title section at the top: 48×48 icon preview (default shell32 #2), "Change Icon…" OFN_EXPLORER picker (*.ico only), and title edit field auto-filled from the project name. Preview popup (`WS_POPUP | WS_CAPTION`, no × button) shows a styled facsimile with a read-only RichEdit (word-wraps to control width via `EM_SETTARGETDEVICE(NULL,0)` — no horizontal overflow) and Back/Next/Cancel buttons; Back ◀ / Next ▶ navigate through all active (visible + enabled) dialog types, Next becomes "Finish ✔" at the last page; clicking Finish shows an "End of Preview" modal (disabled checked "Open \<AppName\>" checkbox + OK) before closing the preview. A floating sizer panel (180 logical px wide; Width/Height spinners, 200–1400/150–1000 logical px) to the left of the preview resizes it live; dimensions are DPI-scaled and persisted to the DB. The sizer also has a **Header font** section (`IDC_IDLG_SZR_FONT_GLOBAL 7128 – IDC_IDLG_SZR_FONT_ITALIC 7133`) — font name edit with a **"…" browse button** (`IDC_IDLG_SZR_FONT_BROWSE = 7137`) that opens `ChooseFontW` pre-seeded with current settings, size spinner (0–72 pt), Bold/Italic checkboxes — and a **Header colors** section (`IDC_IDLG_SZR_CLR_GLOBAL 7134 – IDC_IDLG_SZR_CLR_BG 7136`) — two owner-drawn color swatch buttons (title fg / header bg) showing the effective color; left-click opens the system color picker, right-click resets to `IDLG_NOCOLOR`; each group has an independent "Use on all dialogs" checkbox. The Components page auto-fits its height on open using `EM_FORMATRANGE` content measurement (`MeasureRichEditLogHeight()`); both the no-RTF layout and the split RTF+items layout are supported. Once the developer adjusts the size via the sizer, auto-fitting is suppressed for that project (`s_previewUserSized`, persisted to DB). Module in `dialogs.h/.cpp`; documented in `dialogs_INTERNALS.txt`
- **Dependencies Page**: New modular page for declaring external dependencies required by the installer — ListView with Name / Delivery / Required / Detection columns, Add / Edit / Remove buttons with right-click context menu. Full edit dialog with progressive disclosure per delivery type: Bundled → Required · Install step · Component linkage (optional: `IDC_DEPDLG_COMP_EDIT = 426` + `…` picker `IDC_DEPDLG_COMP_PICK = 427`, visible only when project has components, empty = install always) · Exit codes (optional: space-separated codes treated as success in addition to 0; reboot codes 3010/1641 detected by SetupCraft, user prompted before reboot) · License (optional) · Credits (optional); Auto-download → Required · Install step · Component linkage · Detection (optional: registry key, file path, version check source (`IDC_DEPDLG_VER_SOURCE = 428`, combo: no check / registry key value / file version resource — disambiguates where min/max version is read from), minimum version (`IDC_DEPDLG_MIN_VER = 412`), maximum allowed version (`IDC_DEPDLG_MAX_VER = 424`, for deps incompatible with versions that are too new)) · Network (URL, silent args (optional), SHA-256 (optional), offline behaviour, download timeout in seconds (`IDC_DEPDLG_TIMEOUT = 422`, `ES_NUMBER` edit, 0 = no timeout / wait forever)) · Exit codes (`IDC_DEPDLG_EXIT_CODES = 423`, space-separated; help button `IDC_DEPDLG_EXIT_CODES_HELP = 425` opens a reference popup with common codes via shell32.dll #23 icon) · License (optional) · Credits (optional); Redirect URL → same as Auto-download but Network shows only URL + offline; Instructions only → Required · Install step · Component linkage · Instructions (row-wrapped icon grid, each page editable/removable, stored in `dep_instructions` DB table). "Before the Welcome screen (silent)" install step is automatically removed from the combo when Instructions Only is selected. Add vs. Edit title awareness (`dep.id == 0`). Dialog fonts use `SPI_GETNONCLIENTMETRICS` at the same scale as the main window (×1.2 body, ×1.5 semi-bold title). Dialog positioned near the main window title bar. Scrollable dialog (`WS_VSCROLL`) clamped to work area. Validation errors shown via the styled `ShowValidationDialog()` (i18n-correct). All state persisted to `external_deps` + `dep_instructions` DB tables on Save. Custom scrollbars (V + H) via `my_scrollbar`. Module in `deps.h/.cpp`; documented in `deps_INTERNALS.txt`
- **Shortcuts Page**: Dedicated page for configuring installer shortcuts — Desktop shortcut (with opt-out checkbox, multiple shortcuts, 16×16 mini-icon strip below the large icon), Start Menu & Programs folder hierarchy (editable TreeView with add/remove subfolders, inline rename, custom hover tooltip, "Add Shortcut Here" button and context menu item), Pin to Start, Pin to Taskbar. Full module in `shortcuts.h/.cpp`. Right-click context menus on tree and row buttons. All shortcuts and opt-out flags persisted to the `sc_shortcuts` and `sc_menu_nodes` DB tables on Save; restored on project open via `SC_LoadFromDb`. Custom hidden scrollbar (`s_hMsbSc`, `my_scrollbar`) appears when content exceeds the window height; scroll position tracked via `SC_SetScrollOffset`/`SC_GetScrollOffset` so strip controls stay correctly positioned at any scroll offset; `WS_CLIPSIBLINGS` added to all page controls to prevent overdrawing the status bar when scrolled. All pin-strip checkboxes refresh instantly on every shortcut mutation (all 8 paths wired). Individual pin-strip checkboxes write back to `s_scShortcuts` immediately on click so pin state survives page switches; a lightweight `SC_RefreshPinLabels()` helper updates only the pin-button enable states without destroying/recreating checkbox HWNDs (avoids UB from `DestroyWindow` while `WM_LBUTTONUP` is still on the call stack). **Configure Shortcut dialog** (`sc_shortcut_dialog.h/.cpp`): full-width edits for Executable, Shortcut name, Run in folder (with browse buttons), Arguments (`IDC_SCDLG_ARGUMENTS = 5230`, maps to Inno's `Parameters:`), Comment (`IDC_SCDLG_COMMENT = 5231`, maps to Inno's `Comment:` — Explorer/Start Menu hover tooltip), Hot key (`IDC_SCDLG_HOTKEY = 5232` + `IDC_SCDLG_HOTKEY_CLEAR = 5233`, `msctls_hotkey32` control + red Clear button, maps to Inno's `HotKey:` — global keyboard shortcut string e.g. `ctrl+alt+h`), icon preview + picker, Run as administrator checkbox. All fields stored in `ShortcutDef` and persisted to the `sc_shortcuts` DB table. Right-click context menu on Desktop icon button only; pin icon buttons have no context menu.
...
- **Settings Page**: Five sections — **Application**: App name, version, publisher, publisher URL, support URL, AppId GUID (read-only with Regenerate button), app icon picker. **Build Output**: output folder (with Browse button), output filename, compression type (Store/Zip/LZMA/LZMA2), compression level (0–9), solid compression toggle. **Installation**: UAC privilege level (requireAdministrator / asInvoker / highestAvailable), minimum OS version (None/Win7/Win8/Win8.1/Win10/Win11), **Default dir base** combo (`IDC_SETT_INSTALL_BASE = 8040`) — seven choices: `{pf}` (default), `{pf64}`, `{pf32}`, `{localappdata}`, `{commonappdata}`, `{userdocs}`, Custom… with an inline edit (`IDC_SETT_INSTALL_BASE_CUSTOM = 8041`) for any Inno constant or path string, visible only when Custom is selected. `SETT_GetInstallBasePath()` returns the effective token for the compile pipeline. **Installer Languages**: 3-column checkbox grid of 22 Inno Setup languages (English always on and disabled; tick others to enable a language-picker in the installer). Community Swedish translation (`inno/Swedish.isl`, `local=true`) ships alongside `template.iss`. `SETT_GetInstallerLanguages()` returns `std::vector<InnoLangEntry>` (isl name + local flag) for the compile pipeline; standard languages use `compiler:X.isl`, community files use a relative path. Per-project persistence via DB key `installer_langs`. Russian intentionally excluded. (`IDC_SETT_LANG_BASE = 8050`, IDs 8050–8071.) **Uninstall**: allow uninstall toggle, close running applications before install toggle. All settings persisted per-project via `DB::SetSetting`.
