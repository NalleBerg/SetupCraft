...
**Current Release:** Version 2026.03.23.09 (Published: 23.03.2026 09:15)
...
- **Dependencies Page**: New modular page for declaring external dependencies required by the installer — ListView with Name / Delivery / Required / Detection columns, Add / Edit / Remove buttons with right-click context menu. Full edit dialog with 18 fields: delivery type (Bundled / Auto-download / Redirect URL / Instructions only), Required checkbox, architecture (Any / x64 / ARM64 — x86 removed, app is 64-bit only), install step (named dropdown: Before the Welcome screen / After the Welcome dialog / Before install / After main program installs / At a custom dialog step — or leave unset), registry-key and file-path detection, minimum version, conditional Network fields (URL, SHA-256, silent args, offline behaviour), inline RTF license editor ("Edit License…" opens RTF editor), credits, and inline RTF instructions editor ("Edit Instructions…"). Dialog content column 560 px. Scrollable dialog (`WS_VSCROLL`) clamped to work area. All state persisted to `external_deps` DB table on Save. Module in `deps.h/.cpp`; documented in `deps_INTERNALS.txt`
