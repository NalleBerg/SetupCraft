...
- **`dialogs.cpp` — `PopulateExtras` / `LayoutPreviewControls`: Browse button omitted entirely when "Allow change" is unchecked**: Previously, when `s_selectFolderAllowChange` was false the Browse button was always created alongside the path-edit, and only the path-edit was disabled (per the icon-only button rule). The Browse button was still visible even though clicking it would serve no purpose. Fixed: the Browse button is now conditionally created only when `s_selectFolderAllowChange` is true. When false, only the path-edit is pushed to `hCompChecks[0]`. A new `else if (hCompChecks.size() == 1)` branch in `LayoutPreviewControls` gives the path-edit the full available row width so there is no gap where the button used to be.
- **Docs — `API_list.txt` and `API_INTERNALS\` moved to `MyStyleForAllProjects`**: Both the documentation index and the full `API_INTERNALS\API\` + `INTERNALS\` trees are now kept exclusively in `C:\Users\NalleBerg\Documents\C++\Workspace\MyStyleForAllProjects\`. The SetupCraft repo no longer carries its own copy. A `SetupCraft.code-workspace` file (gitignored) opens both SetupCraft and MyStyleForAllProjects side by side in VS Code.
...
- **`test_page.h` / `test_page.cpp` — new module (IDC range 8300–8315)**: Replaces the "Test functionality to be implemented" placeholder stub with a fully functional Test Installer page (page index 7). Controls created directly on the main HWND (same pattern as `scripts.cpp`/`settings.cpp`). UI: *Output folder* edit (`IDC_TEST_OUTPUT_FOLDER = 8301`) with Blue browse button, *Output file* edit (`IDC_TEST_OUTPUT_FILE = 8303`), Green *Run Test (F5)* button (`IDC_TEST_RUN_BTN = 8304`), status label, Blue *Open test folder* button (`IDC_TEST_OPEN_FOLDER_BTN = 8306`), marquee progress bar (`IDC_TEST_PROGRESS = 8307`), step label (`IDC_TEST_STEP_LBL = 8308`), Blue *Details…* button (`IDC_TEST_DETAILS_BTN = 8309`). Public API: `TEST_BuildPage`, `TEST_TearDown`, `TEST_OnCommand`, `TEST_RunTest`. `IDM_BUILD_TEST` (F5) now calls `TEST_RunTest(hwnd)` from any page (was a `MessageBoxW` stub). Smart defaults: `ComputeDefaultTestFolder()` tries `cfg.outputFolder + \test_output`, falling back to `%USERPROFILE%\Documents\SetupCraft\<ProjectName>-Setupfiles\test_output`; `ComputeDefaultTestFilename()` appends `_test` to `cfg.outputFilename` or forms `<NameNoSpaces>_<ver>_Setup_test`. `UniqueFilename(folder, baseName)` appends `(1)`…`(999)` to avoid overwriting existing files. State persisted via `SETT_SetTestOutputFolder`/`SETT_SetTestOutputFilename` (DB keys `test_output_folder`/`test_output_filename`). `CMakeLists.txt` updated. Documented in `test_page_INTERNALS.txt`.
- **`settings.cpp` / `settings.h` — test output fields + public setters**: `SBuildConfig` gains `testOutputFolder` and `testOutputFilename`. New functions `SETT_SetTestOutputFolder` / `SETT_SetTestOutputFilename` call `MainWindow::MarkAsModified()`. Both values saved/loaded in `SETT_SaveToDb`/`SETT_LoadFromDb`; `SETT_Reset` clears them.
- **`issgen.cpp` — default output filename strips whitespace + appends version**: When `cfg.outputFilename` is empty the generator now strips all whitespace from `proj.name` and appends `_<version>_Setup` (e.g. "SetupCraft Suite" 1.2.0 → `SetupCraftSuite_1.2.0_Setup`). Previously used `proj.name + "_Setup"` which could leave spaces. Pure fallback (name empty too) remains `Setup`.
- **`mainwindow.h` — three new public static accessors**: `GetProjectName()`, `GetProjectVersion()`, `GetCurrentProject()` — expose project data to other modules without a direct internal dependency.
- **`ctrlw.h` / `ctrlw.cpp` — `ExpandEscapes` promoted to public**: Was file-static; now declared in `ctrlw.h`. Converts literal `\n`/`\r\n` escape sequences (as stored in locale files) into real newline characters. Available to any module via `#include "ctrlw.h"`.
- **`_follow.ps1` — build timer**: `$script:buildStart` records the start of each build run. At `[DONE]`, prints `⏱  Build time: mm:ss.f` (Cyan). Resets per run alongside the phase tracker.
- **`locale/en_GB.txt`** (both copies): 14 new `test_*` keys — `test_page_title`, `test_page_desc`, `test_run_btn`, `test_open_folder_btn`, `test_status_idle`, `test_status_no_project`, `test_output_folder_lbl`, `test_output_file_lbl`, `test_status_output_lbl`, `test_status_configured`, `test_output_folder_picker_title`, `test_status_running`, `test_details_btn`, `test_details_title`.
- **New documentation**: `test_page_INTERNALS.txt` (architecture, state model, DB keys, smart-default formulae); `API_INTERNALS\INTERNALS\focus_restore_INTERNALS.txt` (WM_SETFOCUS pattern for restoring keyboard focus on re-activation — pitfall of SetFocus in WM_ACTIVATE, multi-editor-type tab system target rule). Both added to `API_list.txt`.

## [2026.05.25.10] - 2026-05-25

### scripts — delete confirmation uses project-style dialog
- **`scripts.cpp` — `ShowConfirmDeleteDialog` replaces `MessageBoxW(MB_YESNO)`**: The "Remove Script" confirmation now uses the project's custom owner-draw dialog (`ctrlw.h`) instead of the native Win32 message box. Fixes three issues: (1) system-locale button labels ("Ja"/"Nei" on Norwegian systems instead of i18n `yes`/`no` keys); (2) literal `\n` in the locale value `scr_delete_confirm` rendered as two characters in `MessageBoxW` rather than a line break — `ShowConfirmDeleteDialog` calls `ExpandEscapes()` internally so `\n` is properly expanded; (3) unstyled native dialog — now uses Green "Yes" / Red "No" owner-draw buttons consistent with all other confirmation dialogs in the project. Removed unused `wchar_t fmt[256]` declaration. Added `msg.resize(wcslen(msg.c_str()))` to trim null padding from `resize()` before passing to the dialog. `#include "ctrlw.h"` added. `s_emptyLocale` static fallback ensures safe call even if `s_pLocale` is null (never happens in practice).

### Build tooling — _follow.ps1 overhaul + makeit.bat self-logging and verbose output
- **`makeit.bat` — self-logging wrapper**: First invocation truncates `makeit.log` to signal a new run to `_follow.ps1`, then re-calls itself with all stdout+stderr redirected into it via `call "%~f0" >> makeit.log 2>&1` and a `_SC_LOGGED` sentinel flag. This means the developer never needs to pipe anything manually — run `makeit.bat`, open `follow.bat` in a second terminal, and watch the build live. `cmake --build build --config %CONFIG% --verbose` added so the full `g++.exe` invocation (all flags, include paths, defines) is written to the log. All `>nul` and `2>&1` suppressions removed from copy/xcopy operations — every file copy is now visible. Section labels inserted throughout: `[PRE]` (kill running process), `[CONFIGURE]` (cmake configure), `[BUILD]` (compile+link), `[COPY-BUILD]` (copy assets to build\), `[PACKAGE]` (assemble SetupCraft\ output folder), `[DONE]` (all steps completed).
- **`_follow.ps1` — complete rewrite**: Run counter resets to 0 on every `follow.bat` launch (first build of the session = Run #1; increments per build, never persists across sessions). New-run detection unchanged: file truncation (`$size < $pos`) or first-ever content (`$run == 0 && $size > 0`). `[ X%] Building CXX object …` lines become compact inline progress bars (`[████████░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░]  32%  » Compiling  scripts.cpp`) using Unicode block characters (`█`/`░`). Phase-transition banners (`┌─── Phase name`) are printed once when entering each new phase — `[PRE]`, CMake configure, compiler, linker, packaging, done — giving a clear visual breakdown of the build timeline. Line-by-line colour: red `✖` for `error:`, yellow `⚠` for `warning:`, DarkCyan for CMake `-- ` messages, green `✔` for successes and `Built target`, white for compile progress bars, Cyan for linker and packaging steps, DarkGray for verbose `g++.exe` command lines. Run banner clears the screen and shows `BUILD RUN #N  yyyy-MM-dd  HH:mm:ss`.
...
- **scripts — drag-to-reorder (`ScrListSubclassProc` ListView subclass)**: The Scripts page `LVS_ICON` ListView now supports drag-to-reorder. The ListView is subclassed with `ScrListSubclassProc`: `WM_LBUTTONDOWN` records the hit item; `WM_MOUSEMOVE` starts the drag after a 4 px threshold (`SetCapture`), tracks the insertion index via `ComputeDropIndex`, and sets a move cursor; `WM_LBUTTONUP` commits the move via `MoveScript` (indices saved *before* `ReleaseCapture()` — see ReleaseCapture note below); `WM_CAPTURECHANGED` cancels a drag interrupted by e.g. Alt+Tab. `LVS_AUTOARRANGE` removed from the ListView style so items don't re-grid after a drop. `sort_order` fields renumbered in-memory; persisted on next project save.
- **scripts — `ReleaseCapture()` / `WM_CAPTURECHANGED` ordering bug fixed**: `ReleaseCapture()` synchronously delivers `WM_CAPTURECHANGED` *before* it returns. If drag state (`s_dragSrcIdx`, `s_dragDropIdx`) was reset inside `WM_CAPTURECHANGED`, calling `ReleaseCapture()` *after* the `doMove` check would still see stale data. Fix: save `srcIdx`/`dropIdx` and set `s_dragging = false` *before* calling `ReleaseCapture()`, so the `WM_CAPTURECHANGED` handler is a no-op.
- **scripts — `"|"` drop indicator via `NM_CUSTOMDRAW`**: During a drag, `SCR_OnNotify` returns `CDRF_NOTIFYPOSTPAINT` from `CDDS_PREPAINT` and calls `DrawDropIndicator(hdc)` from `CDDS_POSTPAINT`, drawing a 3 px blue vertical bar between tiles to show the insertion position.
- **scripts — custom tooltip for Scripts ListView (`scr_list_drag_tip`)**: `WM_MOUSEMOVE` in `ScrListSubclassProc` calls `ShowMultilingualTooltip` with locale key `scr_list_drag_tip` ("Drag to alter order") when not dragging; `WM_MOUSELEAVE` calls `HideTooltip()`. Uses the project's `ShowMultilingualTooltip`/`HideTooltip` system (`tooltip.h`) — Win32 `TOOLTIPS_CLASS` not used. `TrackMouseEvent(TME_LEAVE)` used for reliable `WM_MOUSELEAVE`.
- **script_edit_dialog — `IDC_SCRDLG_EXPAND = 7374` (*Big editor* toggle, `scr_dlg_expand`)**: New checkbox below the Parameters row. When checked, `EnumChildWindows` hides all controls above the toggle (threshold = toggle y), the toggle moves to `SD_PAD_T`, and Scintilla is resized to fill the freed space. When unchecked the dialog reverts to normal layout and `UpdateWhenVisibility()` restores Finish-page controls if needed. In big-editor mode the Finish-page overlay is always suppressed.
- **script_edit_dialog — layout compaction**: `SD_CONTENT_H` reduced 300 → 240 px; `SD_PAD_B` adjusted to 28 px; *Script type:* label row removed from the layout (saves ~22 px vertical space).
- **script_edit_dialog — `IDC_SCRDLG_PARAMETERS = 7372` (Parameters row)**: New *Parameters (passed to the script process, leave empty for none):* label + edit row, positioned between *Working directory* and the Finish-page section. Persisted via new `parameters TEXT DEFAULT ''` column (idempotent `ALTER TABLE` migration). Locale key: `scr_dlg_parameters`.
- **script_edit_dialog — `IDC_SCRDLG_FINISH_CHECKED = 7373` (*Checked by default on the Finish page* checkbox)**: New custom checkbox below the Finish-page label edit, visible only when `SWR_FINISH_OPTOUT` is selected. When checked, Inno emits `checkedonce` on the `[Run]` entry. Persisted via new `finish_checked_by_default INTEGER DEFAULT 0` column (idempotent `ALTER TABLE` migration). Locale key: `scr_dlg_finish_checked`.
- **script_edit_dialog — Finish-page section via overlay (no wasted space)**: Finish-page controls are NOT allocated rows in `contentH`. They are created *after* `SciCreateEditor()` at `yEditorTop + SD_GAP_SM` (higher Win32 Z-order → paint on top of Scintilla). `UpdateWhenVisibility()` shows/hides the controls *and* resizes Scintilla: when shown, Scintilla starts at `yEditorTop + finishH`; when hidden, Scintilla fills the full `SD_CONTENT_H`. No empty gap when Finish-page section is hidden.
...
### settings / issgen — Setup Log section
- **settings — Section 6: Setup Log (`IDC_SETT_SETUP_LOG_ENABLE = 8075`)**: New *Setup Log* section between Uninstall and Code Signing on the Settings page. Enable checkbox *Write setup log to file* (`IDC_SETT_SETUP_LOG_ENABLE = 8075`); all sub-controls disabled while unchecked. Fields: *Log folder:* path edit (`IDC_SETT_SETUP_LOG_FOLDER = 8076`) with Browse button (`IDC_SETT_SETUP_LOG_FOLDER_BTN = 8077`; `shell32.dll` index 4, yellow folder icon); *Log filename:* edit (`IDC_SETT_SETUP_LOG_FILE = 8078`); *Write mode:* combo Overwrite / Append (`IDC_SETT_SETUP_LOG_MODE = 8079`). Sub-controls tracked in `s_hLogControls[8]`; `ApplyLogEnable()` iterates them on checkbox change. Persisted per-project via DB keys `setup_logging`, `setup_log_folder`, `setup_log_filename`, `setup_log_mode`. `SBuildConfig` gains `setupLogging`, `setupLogFolder`, `setupLogFilename`, `setupLogMode` (0=overwrite, 1=append).
- **issgen — `SetupLogging` token + `CopySetupLog()` Pascal proc**: `SetupLogging=yes/no` emitted via `{#SetupLogging}` token in `[Setup]`. When folder or filename is configured, a `CopySetupLog()` Pascal procedure is generated and injected via the `; <<SETUP_LOG_PROC>>` marker (placed before `procedure DeinitializeSetup()` in `[Code]`). Called via `; <<SETUP_LOG_CALL>>` inside `DeinitializeSetup`. *Overwrite mode*: uses Inno's `FileCopy(ExpandConstant('{log}'), dest, False)`. *Append mode*: reads both files with `LoadStringFromFile`, concatenates, writes back with `SaveStringToFile`. When neither folder nor filename is configured, both markers expand to empty strings. `EscapePascalString()` helper doubles single quotes for safe Pascal string literals.
- **locale — 8 new keys in `en_GB.txt`** (both copies): `sett_sec_setup_log`, `sett_setup_log_enable_lbl`, `sett_setup_log_folder_lbl`, `sett_setup_log_folder_picker_title`, `sett_setup_log_file_lbl`, `sett_setup_log_mode_lbl`, `sett_setup_log_overwrite`, `sett_setup_log_append`.
...
## [2026.05.08.14] - 2026-05-08

### Components — Component groups + install types UI

- **db — `install_types` + `group_name` in `ComponentRow`**: Two new fields added to `ComponentRow` (`db.h`). `install_types` (`std::wstring`) holds space-separated install type names for the component (e.g. `L"full compact"`); `group_name` (`std::wstring`) holds an optional Inno component group name (e.g. `L"Core"`). DB migration: idempotent `ALTER TABLE components ADD COLUMN install_types TEXT DEFAULT ''` and `ADD COLUMN group_name TEXT DEFAULT ''` in `InitDb`. `InsertComponent`, `UpdateComponent`, and `GetComponentsForProject` all extended to include both new columns.
- **issgen — `BuildComponentsSection` emits group header rows + `group\comp` names**: When any component has a non-empty `group_name`, the generator emits an extra `Name: "groupName"; Description: "groupName"; Flags: inherited` header row before the first component in that group, and appends `Name: "groupName\compName"` (backslash-separated) to all components sharing that group.
- **mainwindow — `IDC_COMPDLG_GROUP = 312` group combobox in `CompEditDlgProc`**: New *Group:* (`comp_group_label`) label + `CBS_DROPDOWN` combobox added to the Edit Component (file) dialog, positioned after the Description row. Pre-populated with all existing group names across `s_components`; the developer can select an existing group or type a new name. On OK the text is read into `CompDlgData::outGroupName` and written to `s_components[selIdx].group_name`.
- **mainwindow — `IDC_FOLDER_DLG_GROUP = 344` group combobox in `CompFolderEditDlgProc`**: Same group combobox added to the Edit Folder dialog (`CBS_DROPDOWN`, pre-populated with all existing group names). On OK reads into `CompFolderDlgData::outGroupName` and cascades to all children.
- **mainwindow — `IDC_COMP_TYPE_BASE = 2100` install-types custom checkboxes in `CompEditDlgProc`**: The `LVS_EX_CHECKBOXES` ListView previously used for install types is replaced with individual `CreateCustomCheckbox` calls (one per type, IDs `IDC_COMP_TYPE_BASE + i`). Count stored as window property `"CompTypeCount"`. `WM_DRAWITEM` calls `DrawCustomCheckbox(dis)` first. OK handler reads `BM_GETCHECK` on each and builds the space-separated `install_types` string.
- **mainwindow — `IDC_FOLDER_DLG_TYPE_BASE = 2000` install-types custom checkboxes in `CompFolderEditDlgProc`**: Same replacement in the Edit Folder dialog (IDs `IDC_FOLDER_DLG_TYPE_BASE + i`). Count stored as `"FolderTypeCount"`.
- **mainwindow — Required field replaced with custom checkbox in `CompEditDlgProc`**: The native `BS_AUTOCHECKBOX` Required checkbox is replaced with `CreateCustomCheckbox` to match the project's theme-aware checkbox style. `DrawCustomCheckbox(dis)` added as the first check in `WM_DRAWITEM`; `WM_SETTINGCHANGE` wired to `OnCheckboxSettingChange(hwnd)`.
- **mainwindow — Install types + group persistence for virtual-file nodes**: When `snap->fullPath` is empty (pure-virtual nodes have no real disk source), the init block previously could not read `install_types`/`group_name` from a folder row. Fix: (1) the folder-row cascade write-back always iterates `paths` to update file-component rows regardless of `fullPath`; (2) the init block falls back to reading `install_types`/`group_name` from the first matching file-component row when no folder row exists.
- **mainwindow — Recursive cascade of `install_types` + `group_name` to all children**: Write-back on OK (folder edit) now performs a prefix scan over all `s_components` entries whose `source_path` starts with `snap->fullPath + L"\\"`, updating every nested subfolder row and file-component row — not just direct children.
- **locale — 3 new keys in `en_GB.txt`** (both copies): `comp_group_label` (*Group:*), `comp_col_install_types` (*Install Types*), `comp_types_all` (*(all)*).

## [2026.05.08.10] - 2026-05-08

### Scripts — Per-script component linkage
- **script_edit_dialog / scripts / db — Component linkage field (`IDC_SCRDLG_COMP_EDIT = 7375`, `IDC_SCRDLG_COMP_PICK = 7376`)**: New optional row in the Add/Edit Script dialog, visible for **all** script types whenever the project has at least one component defined. Specifies which components must be selected for this script to run; empty = run unconditionally (default — zero behavior change for existing projects). Read-only edit shows current space-separated component names; `…` button reuses `RunPickCompDialog` from `dep_edit_dialog.cpp` (previously `static`, now exposed with a declaration in `dep_edit_dialog.h`). Section absent when no components. Stored in `DB::ScriptRow::required_components` (`std::wstring`). DB migration: `ALTER TABLE scripts ADD COLUMN required_components TEXT DEFAULT ''` (idempotent). `InsertScript` extended to 15 bound params; `GetScriptsForProject` SELECT extended to column 14. `SCR_EditDialog` gains `const std::vector<std::wstring>& compNames` parameter; all three call sites in `scripts.cpp` build the vector from `MainWindow::GetComponents()` (sorted + deduplicated). Locale key: `scr_dlg_comp_link`.
- **db — `InsertScript` column-list fix**: INSERT SQL listed only 12 column names while binding 14 parameters, silently skipping `parameters` and `finish_checked_by_default` in the column list. All 15 columns now explicitly listed, matching the 15 bound parameters.
