...
### Registry page — per-entry flags, registry view selector, GUID field protection, and data syntax hints
...
- **mainwindow — Add/Edit Value dialog syntax hint**: A gray `STATIC` label (`IDC_ADDVAL_HINT = 5078`) sits immediately below the Data edit field and shows the expected format for the selected type — e.g. `"Decimal: 0–4294967295   or   Hex: 0x00000000"` for `REG_DWORD`, `"Hex bytes, space-separated, e.g.  01 2A FF B0"` for `REG_BINARY`. Updates live on `CBN_SELCHANGE` via `GetRegTypeHint()`. Rendered in `RGB(120, 120, 120)` gray via `WM_CTLCOLORSTATIC` ID check. Both `clientH` calculations (Add and Edit paths) include `AV_GAP_DH + AV_HINT_H`.
