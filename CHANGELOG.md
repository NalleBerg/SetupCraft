...
- **Docs — `API_list.txt` and `API_INTERNALS\` moved to `MyStyle`**: Both the documentation index and the full `API_INTERNALS\API\` + `INTERNALS\` trees are now kept exclusively in `C:\Users\NalleBerg\Documents\C++\Workspace\MyStyle\`. The SetupCraft repo no longer carries its own copy. A `SetupCraft.code-workspace` file (gitignored) opens both SetupCraft and MyStyle side by side in VS Code.
