...
void MainWindow::SwitchPage(HWND hwnd, int pageIndex) {
    ...
    int controlIds[] = {
        ...
        IDC_SETT_SOLID, IDC_SETT_UAC_REQADMIN, IDC_SETT_UAC_INVOKER, IDC_SETT_UAC_HIGHEST, IDC_SETT_PRIV_OVERRIDES, IDC_SETT_MIN_OS,
        ...
        IDC_SETT_DIR_EXISTS_WARNING,
        IDC_SETT_SIGN_ENABLE, IDC_SETT_SIGNTOOL_PATH, IDC_SETT_SIGNTOOL_BTN,
        ...
        IDC_SETT_LANG_DETECTION, IDC_SETT_SHOW_LANG_DLG,
        IDC_SETT_PAGE_TITLE,
...
LRESULT CALLBACK CompFolderEditDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE: {
        ...
        // Exclusive — radio-button style: selecting deselects all others in the group
        {
            std::wstring exclLbl = pData->exclusiveLabel.empty()
                ? L"Exclusive (selecting this deselects all others in the group)" : pData->exclusiveLabel;
            CreateCustomCheckbox(hwnd, IDC_FOLDER_DLG_EXCLUSIVE, exclLbl,
...
LRESULT CALLBACK CompEditDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        // Exclusive checkbox — radio-button style: selecting deselects all others in the group
        {
            std::wstring exclLbl = pData->exclusiveLabel.empty()
                ? L"Exclusive (selecting this deselects all others in the group)" : pData->exclusiveLabel;
...
LRESULT CALLBACK MainWindow::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        ...
        case IDM_COMP_TREE_CTX_EDIT: {
            ...
            int exclusiveState = -1;
            for (const auto& path : paths) {
                for (const auto& cmp : s_components) {
                    if (cmp.source_path != path) continue;
                    if (!cmp.dest_path.empty() && cmp.dest_path != section) continue;
                    if (exclusiveState == -1) exclusiveState = cmp.is_exclusive;
                    else if (exclusiveState != cmp.is_exclusive) { exclusiveState = -1; goto excl_mixed_done; }
                    break;
                }
            }
            excl_mixed_done:
            if (exclusiveState == -1 && !snap->fullPath.empty()) {
                for (const auto& cmp : s_components) {
                    if (cmp.source_path != snap->fullPath) continue;
                    if (cmp.source_type  != L"folder")     continue;
                    if (!cmp.dest_path.empty() && cmp.dest_path != section) continue;
                    exclusiveState = cmp.is_exclusive;
                    break;
                }
            }
